# history Feature

## Responsibilities

- Watch history UI placeholder.

## Public Contracts

- Public Dart exports are limited to presentation/history_page.dart through index.dart.

## Public Index Export List

- presentation/history_page.dart

## Forbidden

- Player event construction or storage implementation.
- Generated bridge calls from widgets.
- Cross-feature deep imports; consume another feature through its index.dart.

## Allowed Dependencies

- Shared contracts.
- Documented infrastructure ports.
- Other feature public index.dart files when route composition requires them.

## Required Checks

- flutter analyze
- flutter test
- pwsh -File scripts/local-quality-gate.ps1